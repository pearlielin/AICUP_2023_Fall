...
validation_set = pd.read_csv(
    'final_test.tsv', sep='\t', names=col_names)
...
for row_i, row in tqdm(validation_set.iterrows(), total=validation_set.shape[0]):
    # find set
    ...
    for key in time_format_keys:
        if key in row['input']:
            ...
            while flag:
                ...
                if pos == -1:
                    break
                if row['input'][pos-1] != ' ' or row['input'][pos+len(key):].find('old', 0) > 0:
                    pos = row['input'].find(key, pos+1)
                    if pos == -1:
                        break
                    else:
                ...
                if time_value.isnumeric():
                    ...
                    normalized_output = 'P' + \
                        en_number_dict[time_value] + time_format_dict[key]
                    seq = f"{row['id']}\tDURATION\t{start_pos}\t{end_pos}\t{output}\t{normalized_output}\n"
                    with open(output_file, 'a', encoding='utf-8') as fw:
                        fw.write(seq)
                else:
